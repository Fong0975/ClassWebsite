<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Cadres_Storke.aspx.vb" Inherits="WebPage_Class_Home" %>
...
<html lang="en">
  ...
  <form id="Form1" runat="server">
	...
	<div class="row">
        ...
        <h3>
				事件編輯 
                Class Stroke Edit</h3>
			<p>
				請先選擇您要以何種身分發布此事項(小老師、幹部......)，接著選取事項的類別(作業、考試、提醒)，最後輸入名稱及開始、結束日期，即可完成輸入。您也可以加上其他欄位的描述，使作業更清楚明瞭。</p>
            <p>
				&nbsp;<small>First, you must choose an identity (teacher's assistant, cadres, etc…). Second, you should choose a style (‘homework’, ‘exam’, ‘reminding’). Finally, as long as you enter the title of this stroke, you will be able to release that. You also can fill in other fields to described in more detail.</small>
			</p>
            <br />
            <br />

			<table cellpadding="0" cellspacing="0" 
            style="font-family: 微軟正黑體; font-size: 12pt; color: #333333; vertical-align: middle; border: 3px solid #666666; padding: 5px; width: 100%">
            <tr>
                <td style="text-align: center; border-right-style: dashed; border-right-width: 2px; border-color: #808080; border-bottom-style: solid; border-bottom-width: 1px;">
                    科目名稱</td>
                <td style="border-bottom-style: solid; border-bottom-width: 1px; border-color: #808080">
                    <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:Class %>" 
                        SelectCommand="SELECT * FROM [Cadres] WHERE (([Account_ID] = @Account_ID)) ORDER BY [Cadres_Name]">
                        <SelectParameters>
                            <asp:SessionParameter Name="Account_ID" SessionField="userid" Type="Int32" />
                        </SelectParameters>
                    </asp:SqlDataSource>
                    <asp:DropDownList ID="ddl_InsertCalendar_TimetableName" runat="server" 
                        DataSourceID="SqlDataSource1" DataTextField="Cadres_Name" 
                        DataValueField="Class_ID" AutoPostBack="True">
                    </asp:DropDownList>
                </td>
            </tr>
            <tr>
                <td style="text-align: center; border-right-style: dashed; border-right-width: 2px; border-color: #808080; border-bottom-style: solid; border-bottom-width: 1px;">
                    事項類別</td>
                <td style="border-bottom-style: solid; border-bottom-width: 1px; border-color: #808080">
                    <asp:DropDownList ID="ddl_InsertCalendar_Style" runat="server">
                        <asp:ListItem Value="1">作業</asp:ListItem>
                        <asp:ListItem Value="2">考試</asp:ListItem>
                        <asp:ListItem Value="3">提醒</asp:ListItem>
                    </asp:DropDownList>
                </td>
            </tr>
            <tr>
                <td style="text-align: center; border-right-style: dashed; border-right-width: 2px; border-color: #808080; border-bottom-style: solid; border-bottom-width: 1px;">
                    事項名稱</td>
                <td style="border-bottom-style: solid; border-bottom-width: 1px; border-color: #808080">
                    <asp:TextBox ID="tb_InsertCalendar_StrokeName" runat="server"></asp:TextBox>
                    <asp:RequiredFieldValidator ID="RequiredFieldValidator1" runat="server" 
                        ErrorMessage="必填" Font-Names="微軟正黑體" Font-Size="10pt" ForeColor="Red" 
                        ControlToValidate="tb_InsertCalendar_StrokeName"></asp:RequiredFieldValidator>
                </td>
            </tr>
            <tr>
                <td style="text-align: center; border-right-style: dashed; border-right-width: 2px; border-color: #808080; border-bottom-style: solid; border-bottom-width: 1px;">
                    事項範圍</td>
                <td style="border-bottom-style: solid; border-bottom-width: 1px; border-color: #808080">
                    <asp:TextBox ID="tb_InsertCalendar_StrokeRange" runat="server"></asp:TextBox>
                </td>
            </tr>
            <tr>
                <td style="text-align: center; border-right-style: dashed; border-right-width: 2px; border-color: #808080; border-bottom-style: solid; border-bottom-width: 1px;">
                    事項描述</td>
                <td style="border-bottom-style: solid; border-bottom-width: 1px; border-color: #808080">
                    <asp:TextBox ID="tb_InsertCalendar_StrokeDescription" runat="server" 
            ...
            <tr>
                <td style="text-align: center; border-right-style: dashed; border-right-width: 2px; border-color: #808080; border-bottom-style: solid; border-bottom-width: 1px;">
                    開始日期</td>
                <td style="border-bottom-style: solid; border-bottom-width: 1px; border-color: #808080">
                    <asp:Calendar ID="cal_InsertCalendar_StrokeStart" runat="server"></asp:Calendar>
                </td>
            </tr>
            <tr>
                <td style="text-align: center; border-right-style: dashed; border-right-width: 2px; border-color: #808080">
                    截止日期</td>
                <td>
                    <asp:Calendar ID="cal_InsertCalendar_StrokeEnd" runat="server"></asp:Calendar>
                </td>
            </tr>
        </table>

          <br />
          <asp:Button ID="bt_InsertCalendar_Submit" runat="server" Text="建立" />
		    <asp:Button ID="bt_Back" runat="server" Text="返回" Style="background-color:#A16B47;" OnClientClick="bt_Back_Click" CausesValidation="False" />
